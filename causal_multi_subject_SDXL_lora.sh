...
export OUTPUT_DIR="./outmodel/SDXL/s*dog-v*sunglasses-SDXL-lora-12.18"
...
CUDA_VISIBLE_DEVICES=0 accelerate launch ./causal_guidance_sdxl_lora.py \
  ...
  --local_attention  \
  --merge_subjects  \
  --merged_class_token="a s* dog and a v* sunglasses"  \
  --train_v --train_k \
  --enable_xformers_memory_efficient_attention \
  --gradient_checkpointing \
  --use_8bit_adam  
