...
def resize_and_pad(image, target_width, target_height):
    """
    根据目标宽度和高度调整图像大小，保持宽高比不变，空白部分填充白色。
    """
    ...
    # 创建一个目标尺寸的空白画布，背景色为白色
    new_image = Image.new("RGB", (target_width, target_height), (255, 255, 255))

    # 将缩放后的图像粘贴到空白画布中，居中放置
...
def merge_images(images, target_width=1024, target_height=1024, direction='horizontal'):
    """
    合并多张图像，保持宽高比不变，并将它们合并为目标尺寸的图像。
    
    :param images: 图像路径列表
    :param target_width: 合并后的目标宽度
    :param target_height: 合并后的目标高度
    ...
    if direction == 'horizontal':
        # 水平合并：每张图像的宽度调整为目标宽度的一半
        each_width = target_width // len(images)
        each_height = target_height  # 高度不变，适应目标高度
    elif direction == 'vertical':
        # 垂直合并：每张图像的高度调整为目标高度的一半
        each_height = target_height // len(images)
        each_width = target_width  # 宽度不变，适应目标宽度
    else:
        raise ValueError("Direction must be 'horizontal' or 'vertical'.")

    # 缩放每张图像并填充空白
    resized_imgs = [resize_and_pad(img, each_width, each_height) for img in img_list]

    # 创建一个空白画布，合并图像
    if direction == 'horizontal':
        # 水平合并
        total_width = target_width
        total_height = max([img.height for img in resized_imgs])
        ...
        # 垂直合并
        total_width = max([img.width for img in resized_imgs])
...
class Merge_MultiSubjectDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #         instance_images[i] = instance_images[i].resize((256,1024))
        ...
        instance_image = merge_images(instance_images, target_width=args.resolution, target_height=args.resolution, direction='horizontal')
        instance_image = self.train_resize(instance_image)
        ...
        instance_image.save("./dog_sunglasses_cat_merge_{}.jpg".format(index))
        if not instance_image.mode == "RGB":
            instance_image = instance_image.convert("RGB")
...
def save_new_embed(text_encoder_1, text_encoder_2, modifier_token_id, modifier_token_id_2, accelerator, args, output_dir, safe_serialization=True):
    ...
    learned_embeds_1 = accelerator.unwrap_model(text_encoder_1).get_input_embeddings().weight
    learned_embeds_2 = accelerator.unwrap_model(text_encoder_2).get_input_embeddings().weight
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--rows", default=1, type=int, help="rows of merged images.")
    parser.add_argument("--cols", default=2, type=int, help="cols of merged images.")
            ...
            if args.merge_subjects:
                ...
                with accelerator.accumulate(unet), accelerator.accumulate(mergeMaskmlp), accelerator.accumulate(text_encoder_one), accelerator.accumulate(text_encoder_two), accelerator.accumulate(map_filter):
                    ...
                    if args.local_attention:
                        ...
                        attention_loss_l = [] 
                        token_index_l = [[1, 2, 3], [5, 6, 7, 8]]#, [9, 10, 11], [13, 14, 15]
                        ...
                        if args.local_attention:
                            ...
                            for i in range(args.train_batch_size): # 1 batch
                                for res in res_list:
                                    ...
                                    #cross attention image corresponding to the specific token
                                    #image = (resize_cross_attention_map(cross_attention_map, token_index[0], (512, 512)) + resize_cross_attention_map(cross_attention_map, token_index[1], (512, 512))) / 2
